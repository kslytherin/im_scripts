...
fig_dir = "/Users/kksmith/figures/run_compare_gmms/"
ftype = ".png"
tick_fontsize = 10
ax_fontsize = 12
leg_fontsize = 10
...
# CAV comparison
    ...
    main_leg_labels = ["LM22-P20", "M21-B14", "CB19", "SA17"]
...
for e_i, mag_num in enumerate(eqmagset):
    ...
    if myfig_ops["makefigs"]:
        # ,layout="compressed"
        fig, ax = plt.subplots(
            2, 2, sharex="col", squeeze=True, layout="constrained", figsize=(7.2, 7.2)
        )
        # breakpoint()
        for st_i, stat_type in enumerate(stat_sets):
            ...
            if simple_stat_type_str[st_i] != "mean":
                ...
                ax[ax1, ax2].hlines(
                    0, dist_min, dist_max, color="lightgrey", linestyle="--"
                )
            # plt.axhline(0, color="lightgrey", linestyle="--")
            ...
            if st_i == 1:
                ax[ax1, ax2].legend(
                    loc="lower left", fontsize=leg_fontsize, fancybox=True
                )
            ax[ax1, ax2].tick_params(
        ...
        # )
        fname = str(myimt) + "_EQ_" + str(e_i) + custtag + ftype
